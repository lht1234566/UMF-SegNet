...
#     # Format the time in a filename-friendly format
...
class MoEFFN_Gating(nn.Module):
    ...
    #     # Get the log file path with current time
    #     log_file_path = get_log_file_path()
    #
    #     # Convert weights to string for logging
    #     weight_str = ', '.join([f"expert {i + 1}: {w:.4f}" for i, w in enumerate(weights.squeeze().flatten().tolist())])
    #
    #     # Open the log file, append the weights, and close the file
...
class SwinTransformerBlockEncoder(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, num_experts,  window_size=7, shift_size=0,
        ...
        # encoder
        self.mlp = MoEFFN_Gating(dim=dim, hidden_dim=mlp_hidden_dim, num_experts=num_experts)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, num_experts, window_size,
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads,
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            SwinTransformerBlockDecoder(dim=dim, input_resolution=input_resolution,
                                        num_heads=num_heads,
                                        num_experts=num_experts,
                                        window_size=window_size,
                                        shift_size=0 if (i % 2 == 0) else window_size // 2,
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=[],
        ...
        num_experts = int(len(num_classes)) if num_classes is not None else 0
        self.num_experts = num_experts
    ...
    def up_x4(self, x, predict_head):
        ...
        if self.final_upsample == "expand_first":
            ...
            max_channels = 0
            for i in range(B):
                output_layer = self.outputs[str(0)]
                output_i = output_layer(x[i].unsqueeze(0))
                output.append(output_i)
                if output_i.size(1) > max_channels:
                    max_channels = output_i.size(1)

            # Pad the outputs to the same number of channels
            for i in range(B):
                padding = max_channels - output[i].size(1)
                output[i] = F.pad(output[i], (0, 0, 0, 0, 0, padding))

                # print("output_i.shape:", output[i].shape)
    ...
    def flops(self):
        ...
        flops += self.num_features * self.num_classes[0]
        return flops
