...
class Universal_model(nn.Module):
    ...
    def forward(self, x_in):
        ...
        if self.encoding == 'rand_embedding':
            task_encoding = self.organ_embedding.weight.unsqueeze(2).unsqueeze(2)
        elif self.encoding == 'word_embedding':
            task_encoding = F.relu(self.text_to_vision(self.organ_embedding))
            task_encoding = task_encoding.unsqueeze(2).unsqueeze(2)
        # task_encoding torch.Size([31, 256, 1, 1, 1])
        x_feat = self.GAP(dec4)
        b = x_feat.shape[0]
        logits_array = []
        for i in range(b):
            x_cond = torch.cat([x_feat[i].unsqueeze(0).repeat(self.class_num,1,1,1), task_encoding], 1)
            ...
            N, _, D, H = head_inputs.size()
            head_inputs = head_inputs.reshape(1, -1, D, H)
            ...
            logits_array.append(logits.reshape(1, -1, D, H))
        
        out = torch.cat(logits_array,dim=0)
        # print(out.shape)
        return out
if __name__ == "__main__":
    network = Universal_model(img_size=(64,64), in_channels=3, out_channels=1)
