...
class VNet_AMC(nn.Module):
    ...
    def __init__(self, n_channels=3, n_classes=1, n_filters=16, normalization='instancenorm', has_dropout=False, n_branches = 3):
        ...
        for i in range(n_branches):
            if has_dropout:
                seq = nn.Sequential(
                    ...
                    nn.Dropout2d(p=0.5),
                    nn.Conv2d(n_filters, n_classes, 1, padding=0)
                )
            else:
                seq = nn.Sequential(
                    ConvBlock(1, n_filters, n_filters, normalization=normalization),
