...
class SalEval(object):
    ...
    def add_batch(self, predict, gth):
        ...
        for t in range(self.nthresh):
            ...
            self.recall[t] += np.sum(intersection * 1. / (np.sum(np.sum(gth, axis=1), axis=1) + np.finfo(np.float64).eps))
            self.precision[t] += np.sum(intersection * 1. / (np.sum(np.sum(bi_res, axis=1), axis=1) + np.finfo(np.float64).eps))
    ...
    def get_metric(self):
        ...
        F_beta = (1 + 0.3) * tp * tr / (0.3 * tp + tr + np.finfo(np.float64).eps)

        return np.max(F_beta), self.thresh[np.argmax(F_beta)]
